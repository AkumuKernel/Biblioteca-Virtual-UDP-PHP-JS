...
$result = $collection->find( [  ] );
  ...
  if(isset($_SESSION["user"])){
    ...
    <main class="container">
      <h2>Lista de Libros que tienes en tu poder</h2>
      <div class="grid grid3">
         <?php foreach ($result as $entry) {
           if($entry["taken"] === $_SESSION["user"]["mail"]){
           ?>
          <div class="card shadow">
            <a <?php echo 'href="solicitud.php?id='.$entry["_id"].'"'?>><img class="book" <?php if(isset($entry["photo"])){echo "src='../{$entry["photo"]}'";}else{echo "src='../img/book.png'";} ?> alt=""></a>
